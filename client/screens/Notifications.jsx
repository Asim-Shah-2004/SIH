        ...
        <View className="mb-6">
            ...
            {data.length > 0 ? (
                <FlatList
                    ...
                    renderItem={({ item }) => (
                        <TouchableOpacity className="bg-white p-4 rounded-lg mb-4 shadow-md" onPress={() => onPressHandler(item)}>
                            {renderItem(item)}
                        </TouchableOpacity>
                    )}
                />
            ) : (
                <Text className="text-gray-500 text-center">No {title.toLowerCase()} available</Text>
            )}
        </View>
    );
