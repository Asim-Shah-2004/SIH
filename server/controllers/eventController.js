...
export const updateEvent = async (req, res) => {
  try {
    ...
    res.status(400).json({ error: "Invalid event ID or failed to update event", details: error.message });
  }
};
...
export const deleteEvent = async (req, res) => {
  try {
    ...
    res.status(400).json({ error: "Invalid event ID or failed to delete event" });
  }
};
