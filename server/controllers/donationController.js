...
export const addDonationCampaign = async (req, res) => {
  try {
    ...
    res.status(400).json({ error: "Failed to create donation campaign", details: error.message });
  }
};
...
export const updateDonationCampaign = async (req, res) => {
  try {
    const { id } = req.params;
    const updatedCampaign = await DonationCampaign.findOneAndUpdate({ id }, req.body, {
      new: true,
      runValidators: true,
    });
    ...
    res.status(400).json({ error: "Invalid campaign ID or failed to update campaign", details: error.message });
  }
};
...
export const deleteDonationCampaign = async (req, res) => {
  try {
    ...
    res.status(400).json({ error: "Invalid campaign ID or failed to delete campaign" });
  }
};
